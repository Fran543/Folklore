...
var drop = function (info) {
   var o = {
      ...
      open: false,
      html: {
         select: dd.get(info.selector)[0],
         options: dd.get(info.selector + ' option'),
         parent: undefined,
      },
      init: function () {
         //Setup Drop HTML
         this.html.parent = dd.get(info.selector)[0].parentNode
         this.html.drop = dd.template('<div class="drop"></div>')
         this.html.dropDisplay = dd.template('<div class="drop-display">Display</div>')
         this.html.dropOptions = dd.template('<div class="drop-options">Options</div>')
         this.html.dropScreen = dd.template('<div class="drop-screen"></div>')
      ...
      renderDrop: function () {
         var that = this;
         var parentHTML = dd.template('<div></div>')
         this.selected.forEach(function (select, index) {
            var option = that.options[select.index];
            var childHTML = dd.template('<span class="item ' + select.state + '">' + option.html + '</span>')
            var childCloseHTML = dd.template(
